...
@WebServlet(name = "autorizados", urlPatterns = { "autorizados" })
public class Autorizados extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        out.println("<html>");
        out.println("<head><title>Comprobantes Autorizados</title>");
        ...
        try {
            facturaEJBBean.flowAutorizaciones();
            out.println("<h3>Comprobantes Autorizados</h3>");
            printDocumento(out, facturaEJBBean.getDocumentoFindxStatus(ComprobanteEnum.AUTORIZADO.getValue()));
        } catch (Exception ex) {
            ex.printStackTrace();
            out.println(ex.getMessage());
            printDocumento(out, new ArrayList<Documento>());
        }
